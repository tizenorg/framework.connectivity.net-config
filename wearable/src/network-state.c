...
static void __netconfig_pop_3g_alert_poppup(void)
{
	bundle *b = NULL;
...
void netconfig_update_default_profile(const char *profile)
{
	...
	if ((old_profile == NULL || netconfig_is_wifi_profile(old_profile) == TRUE)
			&& netconfig_is_cellular_profile(
					netconfig_default_connection_info.profile) == TRUE)
		__netconfig_pop_3g_alert_poppup();
}
